...
const getGig = async (request, response) => {
    ...
    try {
        const gig = await Gig.findOne({ _id }).populate('userID', 'fullname username country image createdAt email description');
        if (!gig) {
...
const getGigs = async (request, response) => {
    ...
    try {
        ...
        const gigs = await Gig.find(filters).sort({ [sort]: -1 }).populate('userID', 'fullname username cover email description isSeller _id image');
